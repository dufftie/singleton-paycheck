...
const getNettoYearValue = (brutto: number) => {
	...
	const nonTaxablePerMonth = nonTaxablePerYear > 0
			? nonTaxablePerYear / 12
			: 0;
	const taxableSum = brutto < nonTaxablePerMonth ? 0 : brutto - nonTaxablePerMonth;
	const pensionTax = percent(brutto, 2);
	const socialInsuranceTax = percent(brutto, 1.6)
	const incomeTax = (taxableSum * 0.20);
	...
	return (
		<Detail.Metadata>
			<Detail.Metadata.TagList title="Services status">
				<Detail.Metadata.TagList.Item text="Timebase - Set up in settings" color={timebaseColor} />
				<Detail.Metadata.TagList.Item text="Toggl Track - Active" color={togglTrackColor} />
			</Detail.Metadata.TagList>
			<Detail.Metadata.Separator />
			<Detail.Metadata.Link title="Open in browser" target="https://timebase.app/dashboard" text="Timebase" />
			<Detail.Metadata.Link title="" target="https://track.toggl.com/timer" text="Toggl track" />
			<Detail.Metadata.Separator />
			<Detail.Metadata.Label title={`Last time updated: ${moment().format("HH:mm:ss, DD.MM.YYYY")}`}/>
		</Detail.Metadata>
	);
};
...
- ${thisMonthHours.toFixed(1)} hours / Average ${currentAverage.toFixed(1)} hours per day
`;
...
`;
	...
	const targetInfo = `
#### 🎯  Target of the month
- ${targetHours} hours / ${untilTarget > 0 ? untilTarget.toFixed(0) : 0} hours remain
- Average per day: ${targetAverage.toFixed(1)} hours / ${remainingAverage > 0? remainingAverage.toFixed(1): 0 } hours remain
